...
main <- function() {
  ...
  if (is.null(job_name) || is.null(task_id) || is.null(output_base)) {
    stop("Arguments --job-name, --task-id, and --output-base are required.")
  }
  ...
  tryCatch(
    {
      test_susine::run_real_case_task(
        job_name = job_name,
        task_id = task_id,
