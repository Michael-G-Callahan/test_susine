...
with `temperature = 1`. This is implemented by `.cluster_weights_from_hc()` in
`R/run_model.R`.
...
`R/collect_results.R`. Both the run-level `agg_weight_run` table and the
aggregated PIP vector therefore use the same nominee-based weighting:

1. Cluster fits by JSD at threshold `0.15` (complete-linkage hclust + cutree).
2. Within each cluster, pick the single highest-ELBO fit as the nominee.
3. Across nominees, assign `w_rep = softmax(ELBO_rep) / cluster_frequency`,
   then renormalize. Non-nominee fits contribute zero weight to the aggregated
   PIP vector.

Prior to this unification, `.aggregate_cluster_weight()` implemented a
different rule (each cluster received total weight `1/K` with within-cluster
ELBO softmax across all members, ignoring across-cluster ELBO comparisons).
