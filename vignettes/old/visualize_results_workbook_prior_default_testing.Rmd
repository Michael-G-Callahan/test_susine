...
Summary: Requires aggregated outputs for the `prior_default_testing` simulation run. This notebook produces AUPRC, power/FDR, and credible-set plots and saves figures/diagnostic CSVs under the run’s aggregated output directory (with file output enabled by default).
...
save_or_show <- function(p, filename, width = 10, height = 7, dpi = 300, subdir = "auprc_informed") {
...
colored by use_case × sigma_0_2_scalar combination.

```{r combined-color-setup}
# Create combined color variable: use_case_id × sigma_0_2_scalar
# Drop 1/L and 2/L levels (since L=10, these are 0.1 and 0.2 which duplicate other levels)
# Relabel 0.5/L as 0.05 (since L=10)

# sigma_0_2_scalar values to EXCLUDE from plots
sigma_exclude <- c("1/L", "2/L")

# Define a combined color palette
# a_i and b_ii split by sigma_0_2_scalar (4 values each)
# b_i split by gamma_shrink (2 values: 0.4, 0.8)
combined_palette <- c(
  ...
  "b_i - 0.4"   = "#238b45",  # darker green for smaller gamma
  "b_i - 0.8"   = "#74c476",  # lighter green for larger gamma
...
# Create labels for legend (more readable)
combined_labels <- c(
  "a_i - 0.05"  = "SuSiE - σ²₀=0.05",
  "a_i - 0.1"   = "SuSiE - σ²₀=0.1",
  "a_i - 0.2"   = "SuSiE - σ²₀=0.2",
  "a_i - 0.4"   = "SuSiE - σ²₀=0.4",
  "b_i - 0.4"   = "SuSiE+FS - γ=0.4",
  "b_i - 0.8"   = "SuSiE+FS - γ=0.8",
  "b_ii - 0.05" = "SuSiNE - σ²₀=0.05",
  "b_ii - 0.1"  = "SuSiNE - σ²₀=0.1",
  "b_ii - 0.2"  = "SuSiNE - σ²₀=0.2",
  "b_ii - 0.4"  = "SuSiNE - σ²₀=0.4"
)
...
cat("sigma_0_2_scalar values:", paste(sort(unique(confusion_filtered$sigma_0_2_scalar)), collapse = ", "), "\n")
```

## 2.1 AUPRC Informed - All p_star Combined

```{r auprc-informed-all, fig.width=14, fig.height=10}
# Compute AUPRC grouped by all relevant variables (including gamma_shrink for b_i)
auprc_all <- test_susine::compute_auprc_from_confusion(
  confusion_filtered,
  group_vars = c("use_case_id", "y_noise", "annotation_r2", "inflate_match", "sigma_0_2_scalar", "gamma_shrink")
)
...
# For sigma-colored plots: keep all rows including b_i (which has NA sigma_0_2_scalar)
auprc_plot_data_faceted <- auprc_plot_data %>%
  dplyr::filter(!is.na(annotation_r2), !is.na(inflate_match)) %>%
  add_combined_color()

if (nrow(auprc_plot_data_faceted) > 0) {
  # Create plot with combined color (no linetypes)
  p <- ggplot2::ggplot(auprc_plot_data_faceted, 
                       ggplot2::aes(x = y_noise, y = AUPRC, color = use_sigma, group = use_sigma)) +
    ggplot2::geom_line(linewidth = 0.8) +
    ggplot2::geom_point(size = 2.5) +
    ggplot2::scale_color_manual(values = combined_palette, labels = combined_labels) +
    ggplot2::facet_grid(
      inflate_match ~ annotation_r2,
      labeller = ggplot2::labeller(
        .rows = function(x) paste0("inflate_match: ", x),
        .cols = function(x) paste0("annotation_r2: ", x)
      )
    ) +
    ggplot2::labs(
      ...
      y = "AUPRC",
      color = "Model × Prior Variance"
    ) +
    ggplot2::theme_minimal() +
    ggplot2::theme(legend.position = "right")
  
  save_or_show(p, "auprc_informed_all_pstar.png", width = 14, height = 10, subdir = "auprc_informed")
}
```
...
for (pstar in pstar_values) {
  ...
  if (nrow(auprc_pstar_faceted) > 0) {
    # Create plot with combined color (no linetypes)
    p <- ggplot2::ggplot(auprc_pstar_faceted, 
                         ggplot2::aes(x = y_noise, y = AUPRC, color = use_sigma, group = use_sigma)) +
      ggplot2::geom_line(linewidth = 0.8) +
      ggplot2::geom_point(size = 2.5) +
      ggplot2::scale_color_manual(values = combined_palette, labels = combined_labels) +
    ...
    save_or_show(p, fname, width = 14, height = 10, subdir = "auprc_informed")
  }
}
```

## 2.3 AUPRC Informed by inflate_match - All p_star Combined

These plots show AUPRC vs y_noise, faceted by annotation_r2 and sigma_0_2_scalar,
colored by use_case × inflate_match combination.

```{r auprc-informed-by-inflate-all, fig.width=14, fig.height=10}
# Create color palette for use_case × inflate_match
inflate_match_values <- sort(unique(na.omit(auprc_all$inflate_match)))
inflate_colors <- list()
inflate_labels <- list()

# Define colors for each combination
color_schemes <- list(
  "a_i" = c("#08306b", "#2171b5", "#4292c6", "#6baed6"),   # Blues
  "b_i" = c("#1a5f1f", "#2d7c2d", "#4cb342", "#81c784"),   # Greens
  "b_ii" = c("#7f2704", "#d94801", "#f16913", "#fd8d3c")   # Oranges
)

for (uc in use_case_filter) {
  for (i in seq_along(inflate_match_values)) {
    im <- inflate_match_values[i]
    key <- sprintf("%s - inflate_%s", uc, im)
    if (!is.null(color_schemes[[uc]])) {
      inflate_colors[[key]] <- color_schemes[[uc]][min(i, length(color_schemes[[uc]]))]
      inflate_labels[[key]] <- sprintf("%s - inflate_match=%.1f", uc, im)
    }
  }
}

# Prepare data with inflate_match grouping
auprc_inflate_data <- auprc_plot_data %>%
  dplyr::filter(!is.na(annotation_r2), !is.na(inflate_match)) %>%
  dplyr::mutate(
    use_inflate = sprintf("%s - inflate_%s", use_case_id, inflate_match)
  )

if (nrow(auprc_inflate_data) > 0) {
  p <- ggplot2::ggplot(auprc_inflate_data, 
                       ggplot2::aes(x = y_noise, y = AUPRC, color = use_inflate, group = use_inflate)) +
    ggplot2::geom_line(linewidth = 0.8) +
    ggplot2::geom_point(size = 2.5) +
    ggplot2::scale_color_manual(values = unlist(inflate_colors), labels = unlist(inflate_labels)) +
    ggplot2::facet_wrap(
      ~ annotation_r2,
      labeller = ggplot2::labeller(annotation_r2 = function(x) paste0("annotation_r2: ", x))
    ) +
    ggplot2::labs(
      title = "AUPRC for Informed Models - Colored by inflate_match (All p*)",
      subtitle = paste("Use cases:", paste(use_case_filter, collapse = ", ")),
      x = "y_noise (Noise Level)",
      y = "AUPRC",
      color = "Model × inflate_match"
    ) +
    ggplot2::theme_minimal() +
    ggplot2::theme(legend.position = "right")
  
  save_or_show(p, "auprc_informed_by_inflate_all_pstar.png", width = 14, height = 10, subdir = "auprc_informed")
}
```

## 2.4 AUPRC Informed by inflate_match - By p_star

```{r auprc-informed-by-inflate-pstar, fig.width=14, fig.height=10}
for (pstar in pstar_values) {
  auprc_pstar <- test_susine::compute_auprc_from_confusion(
    confusion_filtered %>% dplyr::filter(p_star == pstar),
    group_vars = c("use_case_id", "y_noise", "annotation_r2", "inflate_match", "sigma_0_2_scalar")
  )
  ...
  auprc_inflate_pstar <- auprc_pstar_plot %>%
    dplyr::filter(!is.na(annotation_r2), !is.na(inflate_match)) %>%
    dplyr::mutate(
      use_inflate = sprintf("%s - inflate_%s", use_case_id, inflate_match)
    )
  
  if (nrow(auprc_inflate_pstar) > 0) {
    p <- ggplot2::ggplot(auprc_inflate_pstar, 
                         ggplot2::aes(x = y_noise, y = AUPRC, color = use_inflate, group = use_inflate)) +
      ggplot2::geom_line(linewidth = 0.8) +
      ggplot2::geom_point(size = 2.5) +
      ggplot2::scale_color_manual(values = unlist(inflate_colors), labels = unlist(inflate_labels)) +
      ...
      ggplot2::labs(
        title = sprintf("AUPRC for Informed Models - Colored by inflate_match (p* = %d)", pstar),
        subtitle = paste("Use cases:", paste(use_case_filter, collapse = ", ")),
        x = "y_noise (Noise Level)",
        y = "AUPRC",
        color = "Model × inflate_match"
    ...
    fname <- sprintf("auprc_informed_by_inflate_pstar_%d.png", pstar)
    save_or_show(p, fname, width = 14, height = 10, subdir = "auprc_informed")
  }
}
```
...
For plots where sigma_0_2_scalar is NOT on x-axis, we show combined use_case × sigma_0_2_scalar colors.

```{r auprc-simple-prep}
# Grouping vars to cycle through (excluding use_case_id which is color)
x_axis_vars <- c("p_star", "L", "y_noise", "annotation_r2", "inflate_match", "sigma_0_2_scalar")
```

```{r auprc-simple-plots, fig.width=12, fig.height=7}
for (x_var in x_axis_vars) {
  if (x_var == "sigma_0_2_scalar") {
    # When sigma is on x-axis, just color by use_case_id (but still group by gamma for b_i)
    group_vars <- c("use_case_id", "gamma_shrink", x_var)
    ...
    # For other x-axis vars, use combined use_case × sigma (or gamma for b_i) color
    group_vars <- c("use_case_id", "sigma_0_2_scalar", "gamma_shrink", x_var)
    ...
    # Split data: rows with valid x_var vs NA x_var (for horizontal lines)
    auprc_with_x <- auprc_simple %>% dplyr::filter(!is.na(.data[[x_var]]))
    auprc_na_x <- auprc_simple %>% dplyr::filter(is.na(.data[[x_var]]))
    
    # Get x-axis range for horizontal lines
    x_range <- range(auprc_with_x[[x_var]], na.rm = TRUE)
    
    # For NA groups, create segment data spanning the x-axis range
    if (nrow(auprc_na_x) > 0) {
      auprc_hlines <- auprc_na_x %>%
        dplyr::mutate(
          x_start = x_range[1],
          x_end = x_range[2]
        )
    ...
    # Custom plot with combined colors
    p <- ggplot2::ggplot(auprc_with_x, ggplot2::aes(
      x = .data[[x_var]], 
      y = AUPRC, 
      color = use_sigma, 
      group = use_sigma
    ...
    # Add horizontal segments for NA x_var groups (e.g., SuSiE baseline) - these will appear in legend
    if (!is.null(auprc_hlines) && nrow(auprc_hlines) > 0) {
      p <- p + ggplot2::geom_segment(
        data = auprc_hlines,
        ggplot2::aes(x = x_start, xend = x_end, y = AUPRC, yend = AUPRC, color = use_sigma),
        linewidth = 0.8,
        linetype = "dashed"
      )
    }
  ...
  save_or_show(p, fname, width = 12, height = 7, subdir = "auprc_simple")
}
```
...
## 4.1 Power vs FDR - All Runs (Combined Coloring)

```{r power-fdr-all, fig.width=14, fig.height=9}
# Compute curves grouped by use_case × sigma (or gamma for b_i) combination
curves_all <- test_susine::compute_power_fdr_curves(
  confusion_fdr, 
  group_vars = c("use_case_id", "sigma_0_2_scalar", "gamma_shrink")
) %>%
  add_combined_color()

p <- ggplot2::ggplot(curves_all, 
                     ggplot2::aes(x = fdr, y = power, color = use_sigma, group = use_sigma)) +
  ggplot2::geom_line(linewidth = 0.8) +
...
save_or_show(p, "power_fdr_all.png", width = 14, height = 9, subdir = "power_fdr")
```

## 4.2 Power vs FDR - Low p_star (1-4)

```{r power-fdr-low-pstar, fig.width=14, fig.height=9}
...
if (nrow(confusion_low) > 0) {
  ...
  save_or_show(p, "power_fdr_pstar_low.png", width = 14, height = 9, subdir = "power_fdr")
} else {
...
```{r power-fdr-high-pstar, fig.width=14, fig.height=9}
...
if (nrow(confusion_high) > 0) {
  ...
  save_or_show(p, "power_fdr_pstar_high.png", width = 14, height = 9, subdir = "power_fdr")
} else {
...
Boxplots of CS metrics (coverage, size, purity) and model power, grouped by p_star and colored by use_case × sigma_0_2_scalar combination.
...
# Join sigma_0_2_scalar and gamma_shrink from run_table
run_sigma <- run_table %>%
  dplyr::select(run_id, sigma_0_2_scalar, gamma_shrink, annotation_r2) %>%
  dplyr::distinct()

effects_filtered <- effects_filtered %>%
  dplyr::left_join(run_sigma, by = "run_id")
...
cat("sigma_0_2_scalar values:", paste(sort(unique(effects_filtered$sigma_0_2_scalar)), collapse = ", "), "\n")
cat("gamma_shrink values:", paste(sort(unique(na.omit(effects_filtered$gamma_shrink))), collapse = ", "), "\n")
```

```{r cs-metrics-boxplots, fig.width=16, fig.height=12}
...
# Function to create CS metrics boxplot panel with combined coloring
create_cs_metrics_plot <- function(effects_data, models_data, plot_title) {
  # Add combined color variable
  effects_data <- effects_data %>% add_combined_color()
  models_data <- models_data %>% add_combined_color()
  
  # Compute limits for each metric
  coverage_lim <- compute_ylim_grouped(effects_data, "coverage", c("p_star", "use_sigma"), 
                                        lower_bound = 0, upper_bound = 1)
  size_lim <- compute_ylim_grouped(effects_data, "size", c("p_star", "use_sigma"), 
                                    lower_bound = 0, upper_bound = Inf)
  purity_lim <- compute_ylim_grouped(effects_data, "purity", c("p_star", "use_sigma"), 
                                      lower_bound = 0.5, upper_bound = 1)
  power_lim <- compute_ylim_grouped(models_data, "power", c("p_star", "use_sigma"), 
                                     lower_bound = 0, upper_bound = 1)
  
  # Coverage boxplot
  p_coverage <- ggplot(effects_data, aes(x = factor(p_star), y = coverage, fill = use_sigma)) +
    geom_boxplot(position = position_dodge(width = 0.85), outlier.shape = NA, coef = 0) +
    stat_summary(fun = mean, geom = "point", shape = 18, size = 1.5, 
                 position = position_dodge(width = 0.85), show.legend = FALSE) +
    scale_fill_manual(values = combined_palette, labels = combined_labels) +
    coord_cartesian(ylim = coverage_lim) +
    labs(title = "Coverage", x = "p* (# causal effects)", y = "Coverage", fill = "Model × Prior") +
    theme_minimal() +
  ...
  # Size boxplot
  p_size <- ggplot(effects_data, aes(x = factor(p_star), y = size, fill = use_sigma)) +
    geom_boxplot(position = position_dodge(width = 0.85), outlier.shape = NA, coef = 0) +
    stat_summary(fun = mean, geom = "point", shape = 18, size = 1.5, 
                 position = position_dodge(width = 0.85), show.legend = FALSE) +
    scale_fill_manual(values = combined_palette, labels = combined_labels) +
    coord_cartesian(ylim = size_lim) +
    labs(title = "CS Size", x = "p* (# causal effects)", y = "Size", fill = "Model × Prior") +
    theme_minimal() +
    theme(legend.position = "none",
          panel.background = element_rect(fill = "white", color = NA),
          plot.background = element_rect(fill = "white", color = NA))
  ...
  # Purity boxplot
  p_purity <- ggplot(effects_data, aes(x = factor(p_star), y = purity, fill = use_sigma)) +
    geom_boxplot(position = position_dodge(width = 0.85), outlier.shape = NA, coef = 0) +
    stat_summary(fun = mean, geom = "point", shape = 18, size = 1.5, 
                 position = position_dodge(width = 0.85), show.legend = FALSE) +
    scale_fill_manual(values = combined_palette, labels = combined_labels) +
    coord_cartesian(ylim = purity_lim) +
    labs(title = "Purity", x = "p* (# causal effects)", y = "Purity", fill = "Model × Prior") +
    theme_minimal() +
    theme(legend.position = "none",
          panel.background = element_rect(fill = "white", color = NA),
          plot.background = element_rect(fill = "white", color = NA))
  ...
  p_power <- ggplot(models_data, aes(x = factor(p_star), y = power, fill = use_sigma)) +
    geom_boxplot(position = position_dodge(width = 0.85), outlier.shape = NA, coef = 0) +
    stat_summary(fun = mean, geom = "point", shape = 18, size = 1.5, 
                 position = position_dodge(width = 0.85), show.legend = FALSE) +
    scale_fill_manual(values = combined_palette, labels = combined_labels) +
    coord_cartesian(ylim = power_lim) +
    labs(title = "Power", x = "p* (# causal effects)", y = "Power", fill = "Model × Prior Variance") +
    theme_minimal() +
  ...
  gridExtra::grid.arrange(
    ...
    ncol = 1, heights = c(0.5, 8.5, 1.5)
  )
}

# Get unique annotation_r2 values from data
annotation_r2_values <- sort(unique(na.omit(effects_filtered$annotation_r2)))
...
for (af in annotation_filters) {
  # Filter data
  if (is.null(af$value)) {
    eff_data <- effects_filtered
    mod_data <- models_filtered
  } else {
    eff_data <- effects_filtered %>% dplyr::filter(annotation_r2 == af$value | is.na(annotation_r2))
    mod_data <- models_filtered %>% dplyr::filter(annotation_r2 == af$value | is.na(annotation_r2))
  }
  ...
  if (save_plots_to_file) {
    filepath <- file.path(plot_dirs$cs_metrics, fname)
    
    # Must open device BEFORE calling create_cs_metrics_plot (gridExtra requires active device)
    plot_saved <- FALSE
    
    # Try ragg first (best for HPC)
    if (requireNamespace("ragg", quietly = TRUE)) {
      tryCatch(
        {
          ragg::agg_png(filepath, width = 16 * 100, height = 12 * 100, res = 100, bg = "white")
          create_cs_metrics_plot(eff_data, mod_data, af$title)
          grDevices::dev.off()
          cat("Saved:", fname, "\n")
          plot_saved <- TRUE
        },
        error = function(e) {
          tryCatch(grDevices::dev.off(), error = function(e2) {})
          cat("Error saving with ragg:", conditionMessage(e), "\n")
        }
      )
    }
    
    # Try Cairo package if ragg failed or not available
    if (!plot_saved && requireNamespace("Cairo", quietly = TRUE)) {
      tryCatch(
        {
          Cairo::CairoPNG(filepath, width = 16 * 100, height = 12 * 100, res = 100, bg = "white")
          create_cs_metrics_plot(eff_data, mod_data, af$title)
          grDevices::dev.off()
          cat("Saved:", fname, "\n")
          plot_saved <- TRUE
        ...
        error = function(e) {
          tryCatch(grDevices::dev.off(), error = function(e2) {})
          cat("Error saving with Cairo:", conditionMessage(e), "\n")
        }
      )
    }
    
    # Try png() with cairo type as last resort
    if (!plot_saved) {
      tryCatch(
        {
          grDevices::png(filepath, width = 16 * 100, height = 12 * 100, res = 100, bg = "white", type = "cairo")
          create_cs_metrics_plot(eff_data, mod_data, af$title)
          grDevices::dev.off()
          cat("Saved:", fname, "\n")
          plot_saved <- TRUE
        ...
        error = function(e) {
          tryCatch(grDevices::dev.off(), error = function(e2) {})
          cat("Error: Cannot save", fname, "\n")
          cat("  Reason:", conditionMessage(e), "\n")
          cat("  Install ragg or Cairo package: install.packages('ragg')\n")
        }
      )
    }
  } else {
    # Only try to display if not saving to file
    create_cs_metrics_plot(eff_data, mod_data, af$title)
  }
}
```
...
if (!is.null(data_matrices_map)) {
  ...
  x_metrics <- test_susine::build_x_metrics_table(
    matrix_paths_df = data_matrices_map %>% 
      ...
      dplyr::distinct(),
    k_ref = 5,         # typical number of causal effects
...
Test all 1-way and cross-axis 2-way GAM models to find which metrics best predict AUPRC.
We restrict 2-way models to cross-axis pairs (Axis A × Axis B) since:
- Axis A (basin size): Measures how big/strong LD neighborhoods are
- Axis B (identifiability): Measures how interchangeable SNP neighborhoods are

This is more efficient and theoretically motivated than testing all pairs.

```{r gam-combinations}
if (!is.null(auprc_with_x) && nrow(auprc_with_x) > 20 && exists("metric_names")) {
  cat("Testing 1-way and cross-axis 2-way GAM combinations...\n\n")
  
  # Show which metrics are available in each axis
  axis_a_available <- intersect(metric_names, test_susine::get_axis_a_metrics())
  axis_b_available <- intersect(metric_names, test_susine::get_axis_b_metrics())
  cat("Axis A metrics available:", length(axis_a_available), "\n")
  cat("Axis B metrics available:", length(axis_b_available), "\n")
  cat("Cross-axis pairs to test:", length(axis_a_available) * length(axis_b_available), "\n\n")
  
  # Test combinations with cross-axis only for 2-way models
  ...
  # Show top cross-axis 2-way additive models by AIC
  cat("\n=== Top 10 Cross-Axis Additive Models (Axis A + Axis B) ===\n")
  gam_results %>%
