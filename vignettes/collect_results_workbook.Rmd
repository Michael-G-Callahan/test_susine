...
```{r shard-collection-note, echo=FALSE, results='asis'}
if (!verbose_file_output) {
  cat("**Note:** `verbose_file_output = FALSE`, so shard CSVs/Parquet files were\n",
      "materialised during the run. You can skip submitting the shard collection job\n",
      "unless you want to re-run validation manually.\n")
}
```

```{r shard-collection-job, eval=verbose_file_output}
collection_job <- list(
...
- `snps_shard-XXX.parquet` (merged SNP-level table for that shard)

Use the table below to verify progress before aggregating.
When `verbose_file_output = FALSE`, these files should already be present
without running the shard collection job.
...
shard_status <- shard_manifest %>%
  mutate(
    ...
    snps_file = file.path(shards_dir, sprintf("snps_%s.parquet", shard_dir)),
    validation_ready = file.exists(validation_file),
    model_ready = file.exists(model_file),
    effect_ready = file.exists(effect_file),
    snps_ready = file.exists(snps_file)
  )

kable(
  shard_status %>%
    select(shard_idx, run_count, validation_ready, model_ready, effect_ready, snps_ready),
...
# 8. Combine SNP-Level Tables

```{r combine-snps}
if (!all(shard_status$snps_ready)) {
  pending <- shard_status$shard_dir[!shard_status$snps_ready]
  stop("Missing SNP shard files for: ", paste(pending, collapse = ", "))
}

snps_files <- shard_status$snps_file[shard_status$snps_ready]
snps_combined <- purrr::map_dfr(snps_files, ~arrow::read_parquet(.x))
cat("Combined", nrow(snps_combined), "SNP rows\n")

snps_output_path <- file.path(combined_dir, "snps_combined.parquet")
arrow::write_parquet(snps_combined, snps_output_path, compression = "zstd")
cat("Saved SNP table to:", snps_output_path, "\n")
```
...
cat("\n  SNP rows:", nrow(snps_combined), "\n")
cat("  SNP columns:", ncol(snps_combined), "\n")
cat("  SNP-level NAs:", sum(is.na(snps_combined)), "\n")
```
...
# 11. Delete Shard Folders (Optional)
...
- `snps_combined.parquet`

**Original shard folders:** `slurm_output/<job_name>/<parent_job_id>/shard-*/`
(Delete by setting `delete_shards <- TRUE` in step 11)
