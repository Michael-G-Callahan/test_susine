...
join_cols_to_exclude <- intersect(setdiff(names(run_map), "run_id"), snps_cols)
...
# Now re-join label if use_case_id was already in SNPs (we still need the label mapping)
if ("use_case_id" %in% join_cols_to_exclude && "label" %in% names(use_case_labels)) {
  label_map_ds <- arrow::InMemoryDataset$create(use_case_labels)
  snps_joined <- snps_joined %>%
    dplyr::left_join(label_map_ds, by = "use_case_id")
}
...
ggplot(pip_curve_data, aes(x = fdr, y = recall, color = curve_label)) +
  geom_line() +
  geom_point(size = 0.8) +
  labs(
