...
if (length(join_cols_to_exclude)) {
  message("SNPs dataset already has columns: ", paste(join_cols_to_exclude, collapse = ", "),
          " — excluding from join to avoid collision.")
...
# Now compute cumulative TP/FP/TN/FN for each threshold (descending pip)
# At threshold t: predict positive if pip >= t
# TP = causal SNPs with pip >= t, FP = non-causal SNPs with pip >= t
# TN = non-causal SNPs with pip < t, FN = causal SNPs with pip < t

confusion_matrix_table <- confusion_agg %>%
  dplyr::group_by(across(all_of(c(grouping_vars, "label")))) %>%
  dplyr::arrange(dplyr::desc(pip_bucket), .by_group = TRUE) %>%
  dplyr::mutate(
    # Cumulative counts from high pip down (predicted positive at threshold = pip_bucket)
    TP = cumsum(n_causal),
    FP = cumsum(n_noncausal),
    # Total positives and negatives in this group
    total_causal = sum(n_causal),
    total_noncausal = sum(n_noncausal),
    # FN and TN are the remainder
    FN = total_causal - TP,
  ...
  dplyr::select(
    ...
    TP, FP, TN, FN, n_runs, total_causal, total_noncausal
  )
...
# Aggregate across any remaining grouping vars to get one curve per use_case_id
# (sum the counts, then recompute metrics)
...
# Compute precision/recall at each threshold, then AUPRC per group
...
informed_auprc <- informed_metrics %>%
  ...
  dplyr::summarise(
    AUPRC = compute_auprc(dplyr::cur_data()),
    .groups = "drop"
  ) %>%
  dplyr::mutate(
