...
Summary: Requires the detailed confusion matrix from `collect_results_workbook.Rmd`. This notebook produces paper-ready plots (power/FDR, AUPRC, annotation-quality panels) and saves figures under the run’s aggregated `plots_paper` directory.
...
if (nrow(auprc_ann_pstar_scale) > 0) {
  ...
  if (nrow(auprc_ann_pstar_scale_x) > 0) {
    ...
    susine_zero_point <- ai_data_scale_x %>%
      dplyr::transmute(
        use_case_id = "b_ii",
        annotation_scale = 0,
        AUPRC = AUPRC,
        pstar_band = pstar_band,
        sigma_0_2_scalar = sigma_0_2_scalar,
        gamma_shrink = NA_real_,
        line_group = paste0("b_ii - ", sigma_0_2_scalar)
      )

    other_data_scale_x <- dplyr::bind_rows(other_data_scale_x, susine_zero_point)
