---
title: "SuSiNE Pilot — Phase A (Screening)"
output:
  html_document:
    toc: true
    toc_depth: 2
...
Summary: Phase A of the two-phase pilot. Runs all 10 use cases with
single fits plus small c/tau grids for functional use cases. Screens
which use cases and grid dimensions merit exploration budget in Phase B.
See `refs/pilot_compute_optimization_plan.md` for rationale.

```{r setup, message=FALSE}
here::i_am("vignettes/simulation pipeline/run_control_workbook_phase_a.Rmd")
library(here)
here()
...
job_inputs <- list(
  ...
  # All 10 use cases — screening to decide which survive to Phase B.
  use_case_ids = c(
    "susie_vanilla",
    "susie_ash_fixed",
    "susie_ash_eb",
  ...
  # PHASE A: Single fits + small c/tau grids for functional use cases.
  # Non-functional use cases only get "single"; c_grid and tau_grid are

  # filtered to functional_mu and functional_pi respectively by the harness.
  exploration_methods = c("single", "c_grid", "tau_grid"),
  exploration_mode = "separate",
  K = 3L,

  # Core simulation settings (unchanged from pilot_study_v1).
  L_grid = 10L,
  ...
  # Annotation quality grid: trimmed from 4x3 to 3x3.
  annotation_r2 = c(0.0, 0.3, 0.6),
  inflate_match = c(0.7, 1.0, 1.3),

  # Small grids for functional use cases (K=3).
  # c_grid: covers 0 (no directional prior) through moderate c.
  # tau_grid: covers weak through strong functional pi concentration.
  # restart/refine not in exploration_methods but must match K for validation.
  c_grid_values = seq(0, 0.6, length.out = 3),
  ...
  # Max iterations: 50 for Phase A screening (default 100).
  # Cuts wall time on slow outliers ~in half; sufficient for screening.
  max_iter = 50L,

  # Prior variance settings (unchanged).
  ...
  # Pilot matrix selection (same 10 M1-stratified matrices).
  ...
  # Aggregation: minimal for single-fit screening (no ensembles).
  aggregation_methods = c("max_elbo"),
