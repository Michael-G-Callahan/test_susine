...
Summary: Builds SLURM-ready simulation control files from a parameter grid. It previews dataset bundles/run tables and writes job artifacts (config JSON + SLURM script) used by `inst/scripts/run_task.R`.

```{r setup, message=FALSE}
here::i_am("vignettes/simulation pipeline/run_control_workbook.Rmd")
library(here)
here()
...
-   During execution, results land in task-local files at output/slurm_output/<job_name>/<parent_job_id>/task-*/flush-*_*.
...
full Cartesian product (only the first phenotype seed in `seeds` is used in that
mode). Use `"intersect"` to "zip" across the grid values with `rep_len`
semantics so you get a small but representative sample of bundles.
...
job_inputs <- list(
  ...
  # "intersect" for a zipped grid (rep_len across values)
  grid_mode = "full",
  ...
  # Task assignment: must be "dataset" (run-based assignment is disabled).
  task_unit = "dataset",
  # When task_unit = "dataset", number of bundles per SLURM task
  bundles_per_task = 1,
  # Seed used to shuffle dataset bundles into tasks (reproducible assignment)
  task_assignment_seed = 1L,
  # Legacy (ignored) when task_unit = "dataset"
  runs_per_task = 3969,
  ...
  # Restart controls for pilot runs (ignored unless the use case requests it)
...
n_seeds <- dplyr::n_distinct(bundles_tbl$phenotype_seed)

bundles_formula <- sprintf(
  "(= %d loci x %d p_star x %d y_noise x %d seeds)",
  n_matrices, n_p_star, n_y_noise, n_seeds
)
...
group_counts <- runs_tbl %>%
  group_by(use_case_id, dataset_bundle_id) %>%
  summarise(n_groups = dplyr::n_distinct(group_key), .groups = "drop") %>%
  group_by(use_case_id) %>%
  summarise(min_groups = min(n_groups), max_groups = max(n_groups), .groups = "drop")

run_counts <- runs_tbl %>%
  group_by(use_case_id, dataset_bundle_id, group_key) %>%
  summarise(n_runs = dplyr::n(), .groups = "drop") %>%
  group_by(use_case_id) %>%
  summarise(min_runs = min(n_runs), max_runs = max(n_runs), .groups = "drop")

uc_counts <- uc_counts %>%
  left_join(group_counts, by = "use_case_id") %>%
  left_join(run_counts, by = "use_case_id")

cat(sprintf("Number of dataset bundles: %d %s\n", n_bundles, bundles_formula))
cat(sprintf("Number of tasks: %d %s\n", n_tasks, tasks_formula))
cat(sprintf("# of use cases: %d\n", n_use_cases))
...
per_uc_terms <- uc_counts %>%
  mutate(term = sprintf("%s: %d groups x %d runs = %d",
...
Each row is a unique dataset bundle (matrix + sim settings + phenotype seed).

```{r preview-dataset-bundles}
job_config$tables$dataset_bundles %>%
  dplyr::left_join(
    job_config$tables$data_matrices %>%
      dplyr::select(matrix_id, data_scenario, dataset_label),
    by = "matrix_id"
  ) %>%
  select(dataset_bundle_id, data_scenario, matrix_id, dataset_label,
         y_noise, p_star, phenotype_seed) %>%
  arrange(data_scenario, matrix_id, dataset_bundle_id) %>%
  slice_head(n = 20) %>%
  kable()
```
...
run_summary <- summarise_job_config(job_config) %>%
  {
    sort_cols <- intersect(c("use_case_id", "L", "y_noise", "p_star"), names(.))
...
For quick local validation, shrink the grid and run a single task
locally. Two lightweight options are `grid_mode = "minimal"` (one bundle
per scenario) or `grid_mode = "intersect"` (zipped grid across settings).

```{r smoke-test-example}
job_inputs_smoke <- modifyList(job_inputs, list(
  grid_mode = "minimal", # or "intersect"
  task_unit = "dataset",
  bundles_per_task = 1,
  seeds = 1,
  use_cases = head(job_inputs$use_cases, 1)
))
```

To use the smoke settings, replace `job_inputs` with `job_inputs_smoke`
and re-run sections 2-4 before testing locally.
