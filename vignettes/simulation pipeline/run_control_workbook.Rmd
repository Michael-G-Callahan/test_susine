...
job_inputs <- list(
  ...
  # Which prior specs to include (see prior_spec_catalog())
  use_cases = c("susie_vanilla", "susie_functional_pi", "susine_functional_mu"),
  exploration_methods = c("restart", "c_grid", "tau_grid"),
  ...
  # Exploration grids
  c_grid_values = c(0, 0.25, 0.5, 0.75, 1.0),
  tau_grid_values = c(0.25, 0.5, 1, 2, 4),
  ...
  aggregation_methods = c("elbo_softmax", "cluster_weight"),
...
job_config$tables$use_cases %>%
  ...
  select(use_case_id, label, backend, prior_mean_strategy, prior_variance_strategy, inclusion_prior_strategy) %>%
  kable()
```
