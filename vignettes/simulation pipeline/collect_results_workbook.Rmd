...
Summary: Requires a completed simulation run with run_history and staged outputs. This notebook aggregates staging outputs, optionally builds a parquet `snps_dataset`, and writes detailed and dataset-level confusion matrices for downstream analysis.
...
if (snps_dataset_exists && !skip_snps_dataset_aggregation) {
  ...
  message(if (skip_snps_dataset_aggregation) "Staging found; aggregating metrics (skipping snps_dataset parquet)." else "Staging found but not yet aggregated. Will run aggregation.")
}
```
...
uses the aggregated SNPs dataset when available, or will stream the
staging parquet fragments to avoid holding everything in memory.

**Prerequisites:** 
- Section 1 must have been run (to load paths and run_table).
- Either an aggregated SNPs dataset at `paths$snps_dataset_dir` **or**
  staging parquet fragments in `paths$staging_dir`.
...
snps_dataset_exists <- dir.exists(paths$snps_dataset_dir) &&
                       length(list.files(paths$snps_dataset_dir, recursive = TRUE)) > 0
staging_snp_files <- list.files(paths$staging_dir, pattern = "_snps\\.parquet$", recursive = TRUE, full.names = TRUE)
cat("SNPs dataset exists:", snps_dataset_exists, "\n")
cat("Staging parquet fragments available:", length(staging_snp_files), "\n")

if (!snps_dataset_exists && !length(staging_snp_files)) {
  stop("No SNPs dataset or staging parquet fragments found. Run Section 2 first.")
}
...
detailed_grouping_vars <- c(
  ...
  "annotation_r2", "inflate_match", "gamma_shrink", "sigma_0_2_scalar"
)

# Grouping variables for the dataset-level confusion matrix
dataset_grouping_vars <- c("matrix_id", "use_case_id")
```
...
stream_confusion <- !snps_dataset_exists

confusion_detailed <- test_susine::build_confusion_matrix(
  snps_dataset_path = if (snps_dataset_exists) paths$snps_dataset_dir else NULL,
  snp_files = if (stream_confusion) staging_snp_files else NULL,
  run_table = run_table,
