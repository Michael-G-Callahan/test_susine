...
snps_metrics_path <- file.path(here(), "output", "slurm_output", job_name, job_id, "combined", "snps_combined.parquet")
...
snps_metrics <- if (file.exists(snps_metrics_path)) {
  arrow::read_parquet(snps_metrics_path)
} else {
  warning("snps_combined.parquet not found; SNP-level plots unavailable.")
  tibble::tibble()
}
...
if (nrow(snps_metrics)) {
  head(snps_metrics)
}
```

The `snps_metrics` tibble now exposes every SNP’s PIP, truth label, and
per-run metadata, making it straightforward to rebuild the Fig. 2 power
versus FDR curves or the Fig. 3 credible-set summaries directly from a
single source.
