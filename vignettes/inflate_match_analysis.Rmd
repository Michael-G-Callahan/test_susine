...
p <- ggplot(var_long, aes(x = inflate_match, y = variance, color = snp_type)) +
...
save_or_show(p, "01_variance_comparison.png", width = 12, height = 8)
```

## 3.2 Variance Ratio (Non-causal / Causal)

```{r plot-variance-ratio, fig.width=10, fig.height=6}
p <- ggplot(summary_stats, aes(x = inflate_match, y = var_ratio_mean, color = factor(annotation_r2))) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  geom_ribbon(
...
save_or_show(p, "02_variance_ratio.png", width = 10, height = 6)
```
...
p <- ggplot(abs_long, aes(x = inflate_match, y = abs_mean, color = snp_type)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
...
save_or_show(p, "03_abs_mean_comparison.png", width = 12, height = 8)
```
...
p <- ggplot(dist_df, aes(x = mu_0, fill = snp_type)) +
...
save_or_show(p, "04_distribution_overlap.png", width = 14, height = 10)
```
...
p <- ggplot(analysis_df, aes(x = factor(inflate_match), y = var_ratio, fill = factor(annotation_r2))) +
...
save_or_show(p, "05_historical_variance_ratio.png", width = 12, height = 8)
```
...
p <- ggplot(var_comparison, aes(x = factor(inflate_match), y = mu_0_var, 
                           fill = snp_type)) +
...
save_or_show(p, "06_historical_variance_comparison.png", width = 14, height = 8)
```
