...
# Compute AUPRC for each inflate_match level with annotation_scale
...
if (nrow(auprc_inflate_scale) > 0) {
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::labs(
      title = "AUPRC by Annotation Quality, faceted by Inflate Match and Annotation Scale",
      x = "Annotation Quality (causal R^2)",
      y = "AUPRC"
    ) +
    ggplot2::guides(
  ...
  save_or_show(p, "auprc_annotation_r2_by_inflate_match_and_annscale.png", width = 16, height = 12, subdir = "auprc_new")
} else {
  message("No data for annotation_r2 by inflate_match and annotation_scale plot")
}
```
