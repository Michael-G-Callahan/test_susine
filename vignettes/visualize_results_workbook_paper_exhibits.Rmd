...
if (nrow(auprc_ann_pstar_scale) > 0) {
  ...
  ann_scale_palette <- grDevices::colorRampPalette(
    c("#7f0000", "#b30000", "#d94801", "#f16913", "#fdae6b")
  )(length(ann_scale_vals))
  names(ann_scale_palette) <- ann_scale_labels

  other_data_scale_color <- other_data_scale %>%
    dplyr::mutate(annotation_scale_label = factor(annotation_scale_label, levels = ann_scale_labels))
  ...
  p_color <- ggplot2::ggplot() +
    ...
    ggplot2::geom_segment(
      ...
      ggplot2::aes(x = -Inf, xend = Inf, y = AUPRC, yend = AUPRC, color = annotation_scale_label),
      linewidth = 1.0,
      linetype = "dashed"
    ) +
    ggplot2::facet_grid(pstar_band ~ ., scales = "free_y") +
    ggplot2::scale_color_manual(
  ...
  save_or_show(p_color, "auprc_annotation_r2_by_pstar_band_and_annscale_color.png", width = 16, height = 12, subdir = "auprc_new")
} else {
