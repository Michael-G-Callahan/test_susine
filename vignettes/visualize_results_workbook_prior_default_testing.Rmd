...
plot_output_dir <- here::here("output", "visualization_workbook_plots")
...
# 2 use cases × 4 sigma values = 8 combinations
combined_palette <- c(
...
if (nrow(auprc_plot_data_faceted) > 0) {
  ...
  save_or_show(p, "auprc_informed_all_pstar.png", width = 14, height = 10)
}
```
...
for (pstar in pstar_values) {
  ...
  if (nrow(auprc_pstar_faceted) > 0) {
    ...
    save_or_show(p, fname, width = 14, height = 10)
  }
}
```
...
for (x_var in x_axis_vars) {
  ...
  save_or_show(p, fname, width = 12, height = 7)
}
```
...
ggsave(file.path(plot_dirs$power_fdr, "power_fdr_all.png"), p, width = 14, height = 9, dpi = 300, bg = "white")
...
if (nrow(confusion_low) > 0) {
  ...
  ggsave(file.path(plot_dirs$power_fdr, "power_fdr_pstar_low.png"), p, width = 14, height = 9, dpi = 300, bg = "white")
...
if (nrow(confusion_high) > 0) {
  ...
  ggsave(file.path(plot_dirs$power_fdr, "power_fdr_pstar_high.png"), p, width = 14, height = 9, dpi = 300, bg = "white")
