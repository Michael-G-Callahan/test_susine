...
# Filter to rows with valid facet values and add combined color
# Note: Exclude use cases with NA sigma_0_2_scalar (functional sigma strategy) since these
# informed plots are specifically for testing sigma_0_2_scalar parameter values
auprc_plot_data_faceted <- auprc_plot_data %>%
  dplyr::filter(!is.na(annotation_r2), !is.na(inflate_match), !is.na(sigma_0_2_scalar)) %>%
  add_combined_color()
...
for (af in annotation_filters) {
  ...
  if (save_plots_to_file) {
    ...
    tryCatch(
      {
        png(filepath, width = 16 * 100, height = 12 * 100, res = 100, bg = "white")
