...
# Define a combined color palette with enough distinct colors
# 3 use cases × 4 sigma values = 12 combinations
combined_palette <- c(
  ...
  "a_i - 0.2"   = "#4292c6",
  "a_i - 0.4"   = "#6baed6",
  # b_i (SuSiE + functional sigma) - greens (darker = smaller sigma)
  "b_i - 0.05"  = "#1a5f1f",
  "b_i - 0.1"   = "#2d7c2d",
  "b_i - 0.2"   = "#4cb342",
  "b_i - 0.4"   = "#81c784",
...
combined_labels <- c(
  ...
  "a_i - 0.2"   = "SuSiE - σ²₀=0.2",
  "a_i - 0.4"   = "SuSiE - σ²₀=0.4",
  "b_i - 0.05"  = "SuSiE+FS - σ²₀=0.05",
  "b_i - 0.1"   = "SuSiE+FS - σ²₀=0.1",
  "b_i - 0.2"   = "SuSiE+FS - σ²₀=0.2",
  "b_i - 0.4"   = "SuSiE+FS - σ²₀=0.4",
...
# Also filters out excluded sigma levels
add_combined_color <- function(df) {
  df %>%
    dplyr::filter(!sigma_0_2_scalar %in% sigma_exclude) %>%
    dplyr::mutate(
      use_sigma = paste(use_case_id, "-", sigma_0_2_scalar)
    )
}
```
...
if (nrow(auprc_inflate_data) > 0) {
  p <- ggplot2::ggplot(auprc_inflate_data, 
    ...
    ggplot2::facet_grid(
      sigma_0_2_scalar ~ annotation_r2,
      labeller = ggplot2::labeller(
        .rows = function(x) paste0("σ²₀=", x),
        .cols = function(x) paste0("annotation_r2: ", x)
      )
    ) +
    ggplot2::labs(
...
for (x_var in x_axis_vars) {
  if (x_var == "sigma_0_2_scalar") {
    # When sigma is on x-axis, just color by use_case_id
    ...
    # For other x-axis vars, use combined use_case × sigma color
    group_vars <- c("use_case_id", "sigma_0_2_scalar", x_var)
...
# Compute curves grouped by use_case × sigma combination
...
for (af in annotation_filters) {
  ...
  if (save_plots_to_file) {
    ...
    tryCatch(
      {
        # Use cairo backend to avoid X11 requirement
      ...
      error = function(e) {
        # If cairo fails, try regular png as fallback
        tryCatch(
          {
            grDevices::dev.off()
          },
        ...
        cat("Error saving plot:", fname, "-", conditionMessage(e), "\n")
      }
    )
  } else {
    tryCatch(
